...
[Fase 1: UI móvil en WEB]          [Fase 2: SaaS datos]     [Fase 3: Capacitor]    [Fase 4: OTA]
 ...
 ✅ Toolbar calendario móvil + Google Sync en «Más»
...
**Siguiente paso de código:** Fase 1 cierre — HelpManual bottom-sheet (opcional) o inicio Fase 2 (SaaS Firestore).
...
> **Última sesión de implementación:** Sprint 1 — Pasos 0–**4** de la Fase 1 (UI móvil en web).
>
...
| **Paso 3 — Ficha de paciente** | Búsqueda táctil + modales bottom-sheet. Drawer con `bottom` = altura bottom nav. **Validado en producción.** |
| **Paso 4 — Toolbar + Google Sync** | `#calendarToolbar` táctil (Prev/Next, mes, Hoy, filtro terapeuta móvil). Semáforo Google en menú **Más** (`GoogleSyncUI.js`, `#mobileMoreGoogleSync`). `#googleSyncBtn` solo `md+`. |
...
| 4 | Toolbar calendario + Google Sync en «Más» | ✅ Hecho |
