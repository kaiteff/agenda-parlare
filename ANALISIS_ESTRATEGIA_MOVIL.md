...
| Estado | Qué |
|--------|-----|
| ⏳ **Primero** | `firebase deploy --only hosting` (build ya corrido 11 jun). Incluye: cola A0+A (2 jun), fix cuota Parláre, modales paciente móvil. |
| ⏳ **Validar iPhone** | Nuevo paciente / expediente / bitácora: pantalla casi completa, scroll fluido, botones fijos abajo. Panel Costos $300 → nuevo paciente → pagar → Excel columna H = $300. |
| ⏳ **Validar cola (Yari)** | Cancelar cita hoy «debe sesión», filtro ámbar Control Maestro, expediente cola sesión. |
| 🔜 **Siguiente sprint** | Cola prioridad **Fase B** (`WaitlistCopilotPanel` + `space_optimizer`). Ver `PROPUESTA_COLA_PRIORIDAD_PACIENTES_ESPECIALES.md` §10. |

**Resumen sesión 11 jun:** `resumen_sesion/RESUMEN_SESION_20260611.md`

### Cierre de sesión — 11 jun 2026

**Entregado hoy:**
- **Fix cuota Parláre → Google Sheets:** al registrar o configurar $300 ya no cae a $250 en columna Parláre (H). Fallbacks dinámicos vía `AuthManager.getTherapistDefaults()` + `clinicFee` del perfil (`SheetService`, `SyncService`, `CalendarData`, `PatientActions`, `patientService`, `FinancialReport`).
- **Modales paciente — UX iPhone:** modales montados en `document.body` (no quedan atrapados en `overflow-hidden`); altura `92dvh` + safe-area; scroll único con footer fijo; Nuevo Paciente en una columna; bitácora como bottom-sheet; `-webkit-overflow-scrolling: touch`.
...
**Archivos tocados (11 jun):** `js/services/google/SheetService.js`, `js/services/SyncService.js`, `js/services/patientService.js`, `js/modules/calendar/CalendarData.js`, `js/managers/patient/PatientActions.js`, `js/managers/patient/PatientModals.js`, `js/managers/patient/PatientModalsHTML.js`, `js/components/ComponentManager.js`, `js/modules/reports/FinancialReport.js`, `index.css`, `dist/output.css`, `js/modules/help/HelpManual.js`.
...
| 7 | **Copiloto Colaborativo — validar UI** | ✅ Banner **8 min** confirmado (Daniel). ⏳ Opcional: (b) celda ⚡, (c) `copilot_overrides` en Firestore, (d) modal Manual con candidatos. |
...
| 9 | **Quiet Hours + Copiloto** | ✅ 28 may: banner morado (Yari/Diana); cron 8 AM + **Liberar ya** / **Pausar** / **Manual**. Deploy rules + functions + hosting. |
...
| 12 | **Fase 1 Ausencias — validar + Fase 2a en celular** | Tras deploy 28 may: (a) **Más → Vacaciones / Ausencia** abre modal en iPhone (Vero/Sam); (b) 🔒 mini en encabezado día vista semana móvil; (c) con citas afectadas: probar **Cancelar** (sin WA) y **Pasar a terapeuta** (misma hora); (d) aviso rojo si sustituta ocupada a esa hora. |
...
| 25 | **Deploy hosting — cola + cuota + modales móvil (11 jun)** | `npm run build` ✅ hecho. Falta `firebase deploy --only hosting`. Paquete: Fase A0+A (2 jun), fix cuota Parláre→Sheets, UX modales paciente iPhone. |
| 26 | **Validar modales paciente en iPhone (11 jun)** | Tras deploy: **+** Nuevo paciente (scroll completo, Crear fijo abajo), expediente, bitácora bottom-sheet. Sin ventana «mini» ni scroll cortado. |
| 27 | **Validar cuota Parláre $300 → Excel (11 jun)** | Panel Costos → $300 → nuevo paciente → marcar cita pagada → columna H del Excel del terapeuta = 300. |
...
| 16 | ✅ **Limpieza Hosting Storage** (may 2026) | Daniel confirmó revisión en consola Firebase (release storage / revisiones). Cuota Hosting al día. |
| 17 | ✅ **Copiloto — banner 8 min en producción** (may 2026) | Daniel confirmó: banner y contador **8 min** funcionando (alineado con backend `total_wait=480 s`). |

### 🪶 Modo leve — pocas lecturas Firestore (operación diaria)

| Hacer | Evitar |
|-------|--------|
| Una sesión por persona/día; cerrar pestaña al terminar | Dejar Parláre abierto en 3 dispositivos toda la mañana |
| Vista **Día** en celular (menos re-pintado que semana) | Abrir historial de muchos pacientes seguidos (cada uno = `getDocs` extra) |
| Filtro **una terapeuta** (Vero/Sam ven solo la suya) | Admin en «Todas» sin necesidad |
| Vacaciones / bloqueos en un solo batch | Muchos clics «sincronizar» Google manual |
| Revisar dashboard Firebase **1× por semana** | Obsesionarse con el gráfico cada hora |
...
| **Fix cuota Parláre → Sheets (11 jun 2026)** | Cuota del paciente o Panel Costos ya no cae a $250 fijo al sincronizar Excel. `SheetService`, `SyncService`, `CalendarData`, `PatientActions`, `patientService`, `FinancialReport`. |
| **Modales paciente — UX móvil iPhone (11 jun 2026)** | `document.body`, `92dvh`, scroll único + footer fijo, campos una columna, bitácora bottom-sheet. `PatientModalsHTML.js`, `PatientModals.js`, `ComponentManager.js`, `index.css`. |
