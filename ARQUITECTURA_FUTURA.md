...
| 21 Jun 2026 | **Ajuste Manual → App_Data (refuerzo datos existentes)** | `appointmentFinancials.js`, `CalendarData.js`, `SyncService.js`, `PatientActions.js`, `FinancialReport.js` | Lee cita fresca de Firestore al pagar; alinea `clinicFee` con `manualClinicFee` guardado; batch sync y reportes respetan desglose ya en la app. | Revertir `fetchAppointmentForPayment` y `alignClinicFeeSnapshot`. |
