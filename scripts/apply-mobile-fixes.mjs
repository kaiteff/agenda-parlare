import fs from 'fs';

const D = 'di' + 'v';

function fixMotionTags(s) {
    return s.replace(/<\/?motion>/g, (m) => (m.includes('/') ? `</${D}>` : `<${D}`));
}

// 1. ReceptionControl
const rcPath = 'd:/agbc/Ag_Pa/js/modules/reception/ReceptionControl.js';
let rc = fs.readFileSync(rcPath, 'utf8');
rc = rc.replace(
    /\s*\/\/ TAMBIÉN: Añadir al Sidebar[\s\S]*?sidebarReceptionBtn[\s\S]*?\}\);\s*\n\s*\}\s*\n\s*\}/,
    '\n            // Móvil: Control Maestro solo en menú «Más» (MobileBottomNav)\n        }\n    }'
);
fs.writeFileSync(rcPath, rc);

// 2. PatientModals
const pmPath = 'd:/agbc/Ag_Pa/js/managers/patient/PatientModals.js';
let pm = fs.readFileSync(pmPath, 'utf8');
pm = fixMotionTags(pm);

const titleRe = /dom\.patientHistoryTitle\.innerHTML = `[\s\S]*?\$\{recurrenceAlert\}\s*<\/div>\s*`;/;
const newTitle = `dom.patientHistoryTitle.innerHTML = \`
                <${D} class="flex flex-col gap-2 w-full min-w-0 pr-14 md:pr-0">
                    <${D} class="flex items-center gap-3 min-w-0">
                        <${D} class="w-11 h-11 flex-shrink-0 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-lg shadow-sm border border-white">
                            \${patient.name.charAt(0)}
                        </${D}>
                        <${D} class="min-w-0 flex-1">
                            <${D} class="text-base md:text-lg font-extrabold text-gray-900 leading-tight truncate">\${patient.name}</${D}>
                            <${D} class="text-xs text-gray-500 font-medium flex items-center gap-1 truncate">
                                <span class="w-2 h-2 flex-shrink-0 rounded-full \${patient.isActive !== false ? 'bg-green-500' : 'bg-gray-400'}"></span>
                                Terapeuta \${therapistName}
                            </${D}>
                        </${D}>
                    </${D}>
                    <${D} class="flex flex-wrap items-center gap-2">
                        <button type="button" id="patientWelcomeBtn" class="flex items-center gap-1.5 px-2.5 py-1.5 bg-green-100 text-green-700 text-[10px] font-bold uppercase rounded-lg border border-green-200 hover:bg-green-200 transition-colors touch-manipulation" title="Enviar Mensaje de Bienvenida">
                            <svg class="w-3.5 h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                            Bienvenida
                        </button>
                    </${D}>
                    \${recurrenceAlert}
                </${D}>
            \`;`;

if (!titleRe.test(pm)) {
    console.error('title block not found in PatientModals.js');
    process.exit(1);
}
pm = pm.replace(titleRe, newTitle);

// recurrence alert id
pm = pm.replace(
    /const recurrenceAlert = \(isEndingSoon \|\| hasNoFuture\) \? `[\s\S]*?` : '';/,
    `const recurrenceAlert = (isEndingSoon || hasNoFuture) ? \`
                <${D} id="patientRecurrenceAlert" class="flex items-center gap-1.5 py-1.5 px-2.5 bg-red-50 border border-red-100 rounded-lg w-full">
                    <span class="flex h-2 w-2 flex-shrink-0 rounded-full bg-red-600"></span>
                    <span class="text-[10px] font-black text-red-600 uppercase tracking-tight leading-tight">
                        \${hasNoFuture ? 'Sin citas programadas' : 'Recurrencia por agotarse'}
                    </span>
                </${D}>
            \` : '';`
);

fs.writeFileSync(pmPath, pm);

// 3. PatientModalsHTML
const htmlPath = 'd:/agbc/Ag_Pa/js/managers/patient/PatientModalsHTML.js';
let html = fs.readFileSync(htmlPath, 'utf8');
html = fixMotionTags(html);
html = html.replace(
    'flex justify-between items-start gap-3 bg-white flex-shrink-0',
    'flex flex-col md:flex-row md:justify-between md:items-start gap-2 md:gap-3 bg-white flex-shrink-0 relative'
);
html = html.replace(
    'class="flex items-center gap-3 min-w-0 flex-1" id="patientHistoryTitle"',
    'id="patientHistoryTitle" class="min-w-0 w-full md:flex-1"'
);
html = html.replace(
    'id="authDebugInfo" class="hidden text-[8px]',
    'id="authDebugInfo" class="hidden md:flex text-[8px]'
);
html = html.replace(
    'class="flex items-center gap-0.5 flex-shrink-0">',
    'class="flex items-center gap-0.5 flex-shrink-0 absolute top-3 right-3 md:relative md:top-auto md:right-auto z-30 bg-white/95 md:bg-transparent rounded-full md:rounded-none shadow-sm md:shadow-none pl-0.5">'
);
fs.writeFileSync(htmlPath, html);

console.log('done');
