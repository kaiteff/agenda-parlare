...
| 2 jun 2026 | **Campo `schedulingQueue` en `patientProfiles` (cola debe sesión)** | Escritura solo vía reglas existentes de perfiles (`isSuper()` / terapeuta dueño). Sin colección nueva; sin PII extra en logs (`Logger` en `SchedulingQueueService`). UI: Yari/admin en expediente + Control Maestro. |
| 2 jun 2026 | **L-1 — suscriptor glow Copiloto al logout** | `WaitlistCopilotPanel.destroy()` desuscribe glow; `WaitlistCopilotService.stop()` hace `_subscribers.clear()` para evitar callbacks duplicados tras re-login. |
| 2 jun 2026 | **Campo `schedulingQueue` en `patientProfiles` (cola debe sesión)** | Escritura vía reglas existentes (`isSuper()` / terapeuta dueño). Sin colección nueva. `SchedulingQueueService` usa `Logger`, no teléfonos en log. |
| 2 jun 2026 | **L-1 — suscriptor glow Copiloto al logout** | `WaitlistCopilotPanel.destroy()` + `WaitlistCopilotService.stop()` → `_subscribers.clear()`. |
...
| 30 may 2026 | **Error Reporting backend (functions)** | Corregidos: `trigger_utils.change_before_after` (before/after None en create/delete → 150+90 errores receipt/cancel trigger); `release_quiet_hours_offers` usa `db` global (no `_get_db`); `secrets=ALL_SECRETS` en triggers Autopilot; `TWILIO_WHATSAPP_FROM` sin `.value` erróneo. Redeploy `functions` recomendado. |
