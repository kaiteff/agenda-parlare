...
- [ ] **Optimización crítica de lecturas Firestore (Prioridad 1 - costo $0)**: Cambiar consultas en tiempo real de `CalendarData.js` y `PatientManager.js` para evitar descargar las 2,162 citas del historial de golpe. (1) Limitar rango de escucha inicial a `[-30, +60]` días y actualizar la query dinámicamente al navegar la agenda, (2) Cargar historial completo y calcular saldos de pacientes bajo demanda (`getDocs` al abrir ficha en lugar de tener todo en memoria), (3) Filtrar citas en base de datos por terapeuta para Sam/Vero.
...
---
*Última actualización: 25 de Mayo, 2026 (noche) — **Fase 1.5 Ausencias** (UX premium del modal): cards tipo de ausencia con `has-[:checked]`, atajos rápidos (Hoy/Esta semana/Próxima semana/2 semanas), resumen indigo con conteo de días hábiles, banner verde «sin conflictos», validación visual en tiempo real `endHour > startHour`, botón Confirmar con gradient `blue→indigo` e icono candado. **Ejemplo paso a paso «Me voy de vacaciones la próxima semana»** dentro de `HelpManual.js` (cuadro emerald-50, 8 pasos prácticos) + replicado en `MANUAL_TERAPEUTAS.md`. **Pop-up Novedades v9.2** ahora incluye micro CTA «Pruébalo en 3 toques» al final de la tarjeta de Vacaciones, apuntando al manual para el ejemplo completo. Estado heredado: Fase 1 Ausencias completa con hotfixes S-011 a S-014, sweep XSS S-015/S-016, safe-area iPhone, header móvil limpio, Frontend Copiloto Colaborativo. Estado seguridad: 0 críticos, 4 altos, 3 medios, 2 mejoras, 15 reforzados.*
